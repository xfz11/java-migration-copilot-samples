...
clarification questions. It rewrites `plan.md` and `.metadata/tasks.json` in place using the same
template conventions as `create-modernization-plan`.

## User Input

modernization-work-folder (Mandatory): The folder containing the existing plan files (plan.md, .metadata/tasks.json, clarifications.json)
...
1. **Read existing artifacts**: Read the following files from `modernization-work-folder`:
   ...
   - `.metadata/tasks.json` — existing structured task list
...
4. **Rewrite tasks.json**: Update the task list to reflect any changes in task scope, requirements,
   ...
   **Updating vs adding tasks — critical rule**: When a clarification answer changes *how* an
   ...
   clarifications unless the answers introduce genuinely new migration scope. In all cases, you must
   abide by tasks-schema.json during your edits.**
...
5. **Write updated files**: Save the updated `plan.md` and `.metadata/tasks.json` back to `modernization-work-folder`.
